...
export class SafeApiKit {
  ...
  async getSafeDelegates({
    ...
    if (safeAddress) {
      const { address: safe } = await this.#ethAdapter.getEip3770Address(
        safeAddress
      );
      url.searchParams.set("safe", safe);
    }
    if (delegateAddress) {
      const { address: delegate } = await this.#ethAdapter.getEip3770Address(
        delegateAddress
      );
      url.searchParams.set("delegate", delegate);
    }
    if (delegatorAddress) {
      const { address: delegator } = await this.#ethAdapter.getEip3770Address(
        delegatorAddress
      );
  ...
  async addSafeDelegate({
    ...
    const body: any = {
      safe: safeAddress
        ? (await this.#ethAdapter.getEip3770Address(safeAddress)).address
        : null,
      delegate,
      delegator,
      label,
  ...
  async proposeTransaction({
    ...
    const { address: sender } = await this.#ethAdapter.getEip3770Address(
      senderAddress
    );
    if (safeTxHash === "") {
      throw new Error("Invalid safeTxHash");
  ...
  async getNextNonce(safeAddress: string): Promise<number> {
    ...
    if (pendingTransactions.results.length > 0) {
      const nonces = pendingTransactions.results.map((tx: any) => tx.nonce);
