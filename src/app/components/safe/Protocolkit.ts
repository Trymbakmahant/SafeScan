...
export async function safeDeploy({
  ...
  const contractNetworks = CUSTOM_CONTRACTS_CHAINS.includes(chainId)
    ? CUSTOM_CONTRACTS
    : undefined;
  ...
  try {
    ...
    if (!sponsored || !isRelaySupported(chainId)) {
      ...
      const readOnlyProxyFactoryContract =
        await ethAdapter.getSafeProxyFactoryContract({
          safeVersion,
      ...
      const relayResponse: RelayResponse =
        await relayKit.sendSponsorTransaction(
          proxyFactoryAddress,
          createProxyWithNonceCallData,
          chainId
        );

      const transactionHash = await waitForRelayTaskToComplete(
  ...
  if (isRelaySupported(chainId)) {
    ...
    try {
      ...
      if (tx.safeSigner) {
        ...
        const safeSignature = await signTransactionBySafe(
          safe,
          safeSigner,
          standardizedSafeTx
        );
